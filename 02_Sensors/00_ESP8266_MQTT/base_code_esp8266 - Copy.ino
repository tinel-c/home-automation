...
#define WIFI_PASS "[reteaualuitinel]"
...
void loop() {
  ...
  {
    //locally store the ip of the device
    char str_ip[60];
    //publish connection status
    
    const char* buf = " - local ip. I am connected.";
    strcpy(str_ip,WiFi.localIP().toString().c_str());
    strcat(str_ip,buf);
    ...
    message.dup = false;
    message.payload = (void*) str_ip;
    message.payloadLen = strlen(str_ip) + 1;
